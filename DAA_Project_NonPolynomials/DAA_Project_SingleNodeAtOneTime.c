...
int main(){
	...
	if(f!=NULL)                                                     //Checks file is null or not and proceeds if not null
	{
		...
		float coef;float NLoad[N+1][N+1];                                    //defines coef and Lo 2D array to store new load according to our strategy i.e single car ata atime into the system
		for(i=1;i<=N;i++)
			for(j=1;j<=N;j++)
				if(i!=j)
	                                F[i][j]=1;                                      //Assigns flow single car at a time into the system
		for(i=1;i<=N;i++)
			for(j=1;j<=N;j++)
				NLoad[i][j]=0;                                             //Initializes New Load Matrix
                /*Calculates New Load Accoring to Our Strategy*/
		for(i=1;i<=N;i++)
			for(j=1;j<=N;j++)
				for(k=Path[i][j][0];k>1;k--)
                                {
						NLoad[Path[i][j][k]][Path[i][j][k-1]]=NLoad[Path[i][j][k]][Path[i][j][k-1]]+F[i][j]; //Calculates total load if the load from each node is 1 at a time
                                                count++;                                                                             //increments counter by 1
                                }
		float NCM[N+1][N+1],power;                      //New congestion edge matrix
                float Final_Delay[N+1][N+1];
		for(i=1;i<=N;i++)
			for(j=1;j<=N;j++)
			{
				if(NLoad[i][j]<=C[i][j])        //if load is less than capacity we will send new load at a time to the link
				{
				        coef=(C[i][j]+1)/(C[i][j]+1-NLoad[i][j]);power=floor(Load[i][j]/NLoad[i][j]);//calculates coef and power i.e no.of times it congestionedgematrix is calculated
                                        count=count+6;                                                               //increments counter by 6                                
                                }
				else
				{
				        coef=(C[i][j]+1)/(C[i][j]+1-C[i][j]);power=floor(Load[i][j]/C[i][j]);	//if load is greater than capacity then we take load as capcity value at a time
                                        count-count+6;                                                          //increments counter by 6
				}
				NCM[i][j]=pow(coef,power)*E[i][j];                                              //calculates congestion length
                                count=count+2;                                                                  //increments counter by 2
			}                               
		for(i=1;i<=N;i++)
			for(j=1;j<=N;j++)
				if(i!=j)
				        ...
				        if(NLoad[i][j]<=C[i][j])                                                //if load is less than capacity we will send new load at a time to the link
				        {
				             float rem=(Load[i][j]-floor(Load[i][j]/NLoad[i][j])*NLoad[i][j]);  //Find the remaining load to calaculate congestion
				             coef=(C[i][j]+1)/(C[i][j]+1-(Load[i][j]-floor(Load[i][j]/NLoad[i][j])*NLoad[i][j]));//calculates coef
                                             count=count+12;                                                    //increments counter by 12
                                        }
				        else
                                             coef=(C[i][j]+1)/(C[i][j]+1-(Load[i][j]-floor(Load[i][j]/C[i][j])*C[i][j]));
				        NCM[i][j]=coef*NCM[i][j];                                                //calculates congestion length after total flow completion for i,j position
				        Final_Delay[i][j]=NCM[i][j];                                             //Assigns congestion edge matrix to final all pairs shortest matrix as initialization
                                        count=count+8;                                                          //increments counter by 8
				}     
                /*prints Final Congestion Edge Matrix*/
		printf("Congestion Edge Matrix :\n");
		for(i=1;i<=N;i++)
		{
			printf("|");
			for(j=1;j<=N;j++)
			{
				if(NCM[i][j]==INF)
				        ...
				        printf("%.2f\t",NCM[i][j]);                                              //prints final congestion edge matrix
			}
			printf("\t|\n");
		}
		for(i=1;i<=N;i++)
			for(j=1;j<=N;j++)
				if(Load[i][j]==0&&i!=j)                 //if i,j is not having any load then proceeds
                                        Final_Delay[i][j]=0;            //initiliazes 0 if there is no shortest length with single edge length
                /*Calculates Final All pairs Shortest path with Congestion */
		for(i=1;i<=N;i++)
			for(j=1;j<=N;j++)
				for(k=Path[i][j][0];k>1&&i!=j;k--)
						if(Path[i][j][0]>2)
						{
                                                        Final_Delay[i][j]=NCM[Path[i][j][k]][Path[i][j][k-1]]+Final_Delay[i][j]; //calculates final path length with congestion using shortest path
                                                        count++;                                                                 //increments counter by 1
                                                }
		...
		printf("\nFinal Congestion Actual Delay Matrix :\n");
		for(i=1;i<=N;i++)
		{
			printf("|");
			for(j=1;j<=N;j++)
			{
                                if(i==j)
                                        Final_Delay[i][j]=0;
				printf("%.2f\t",Final_Delay[i][j]);                                             //prints final delay matrix
			}
			printf("\t|\n");
		}
                printf("*****************Final Output :*****************\n");
		...
		printf("\nShorest Path between source %d and destination %d is: %.2f.\n",S,D,Final_Delay[S][D]);//prints shortest path between given source and destination
